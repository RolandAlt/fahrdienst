...
// "C:\Program Files\nodejs\node.exe" restore_auth_password.js
// -------------------------------------------------------------------------
// 1) HIER DEIN NEUES SUPABASE-PROJEKT EINTRAGEN
// -------------------------------------------------------------------------
// Datei: restore_auth_password.js
...
async function main() {
  ...
  for (const line of lines) {
    ...
    // Wir brauchen nur id und email aus der CSV
    const [id, email] = line.split(",", 3);
    const trimmedEmail = email.replace(/^"+|"+$/g, ""); // evtl. Anführungszeichen entfernen

    console.log(`\n→ Setze Passwort für User: ${trimmedEmail} (${id})`);

    const body = {
      // Passwort = komplette E-Mail-Adresse
      password: trimmedEmail,
    };

    const res = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${id}`, {
      method: "PUT", // <─ WICHTIG: PUT statt PATCH
      headers: {
    ...
    if (!res.ok) {
      ...
      console.log("   ✅ Passwort gesetzt/aktualisiert");
    }
  }
